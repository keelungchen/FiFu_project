...
# size vs circ colored by p/a ratio plot
```{r}
ggplot(pa_data, aes(x = log10(Shape_Area), y = log10(circ), color = pa_ratio)) +
    geom_point() +  # 添加點圖層
    xlab("log(Size)") +  # 設置X軸標籤
    ylab("log(Circ)") +  # 設置Y軸標籤
    scale_color_gradient(low = "blue", high = "red") # 使用藍色到紅色的漸變   
...
pa_data_done$Discrete_change <- ifelse(pa_data_done$Fate == "D", 0, ifelse(pa_data_done$Fate != "D", 1, NA))
pa_data_done_12 = filter(pa_data_done, Survey_times == 1 | Survey_times == 2)

# Clean data
df <- data.frame(pa_data_done_12$Discrete_change, pa_data_done_12$Shape_Area, pa_data_done_12$pa_ratio, pa_data_done_12$circ) 
colnames(df) <- c("Growth", "Size","PA_ratio","Circularity")

model <- glm(df$Growth ~ df$Size + df$Circularity, data = df, family = binomial)
summary(model)
with(summary(model), 1 - deviance/null.deviance) # Calculate McFadden's R-squared
#summary(model)$coefficients # Extract the results
...
annotation <- data.frame(
  x = c(log10(median(df$Size)),log10(median(df$PA_ratio)),log10(median(df$Circularity))),
  y = c(0.1,0.1,0.1)
)
...
pa_data_done$Discrete_change <- ifelse(pa_data_done$Fate == Fates, 1, ifelse(pa_data_done$Fate != Fates, 0, NA))
pa_data_done_12 <- filter(pa_data_done, Survey_times == 1 | Survey_times == 2)
pa_data_done_12 <- filter(pa_data_done_12, Fate != "D") #Exclude mortality, use when not D
#pa_data_done_12 <- filter(pa_data_done_12, size_class != "J") #Exclude Juvenile
...
```
![Fig3](Figs/Proportion_Fates_Revers_NoJ.png)
...
```
![Fig4](Figs/Multiplier_Fates_boxplot_NoJ.png)
...
```
![Fig5](Figs/Method_compare_aXm.png)

```{r}
########## Relation between size and multiplier

# Define the fates 
Fate_loop <- c("G","S","Fi","Fu")

# Calculate R square and slope
R_square <- numeric(length(Fate_loop))
multiplier_slope <- numeric(length(Fate_loop))

for (i in seq_along(Fate_loop)) {
  df <- filter(pa_data_done, Fate == Fate_loop[i])
  model <- glm(rate_ad ~ log10(Shape_Area), data = df)
  R_square[i] <- with(summary(model), 1 - deviance/null.deviance) # Calculate McFadden's R-squared
  multiplier_slope[i] <- coef(model)[2] # Calculate slope
}

R_square <- round(R_square, digits = 2)
multiplier_slope <- round(multiplier_slope, digits = 2)
...
for (i in seq_along(Fate_loop)) {
  df <- filter(pa_data_done, Fate == Fate_loop[i])
  p <- ggplot(data = df, aes(log10(Shape_Area), rate_ad))+
      geom_point(shape=21)+
      geom_smooth(method="glm")+
      ylab(paste(Fate_loop[i],"Multiplier"))+ 
      geom_text(label=paste("R² =", R_square[i]), 
                x=Inf, y=Inf, hjust=1.1, vjust=1.1)
  plot_list[[i]] <- p
}

# 使用cowplot套件繪製多個圖形
fig6_1 <- plot_grid(plot_list[[1]], plot_list[[2]], plot_list[[3]], plot_list[[4]],
                    align="h", labels=c("A","B","C","D"), nrow = 4)
ggsave("C:/Users/keelu/R/FiFu_analysis/Figs/multiplier_size_R2.png", plot = fig6_1, width = 3, height = 8, dpi = 100)
fig6_1
```
        ...
        for (i in 1:length(unique(pa_data_done_2$ID))) {
            ...
            if (pa_data_done_1$Fate[j] == "S" | pa_data_done_1$Fate[j] == "G"){
              if (current_id == pa_data_done_2$ID[i]){ # check the ID is match
                ...
                morpho_dynamic$rate_ad[j] <- pa_data_done_1$rate_ad[j]
...
```{r}
############ check the differences of multipliers between dynamics
dynamics_df$dynamic <- as.factor(dynamics_df$dynamic)
dynamics_df$stage <- as.factor(dynamics_df$stage)
summary(dynamics_df)
write.csv(dynamics_df, file = "C:/Users/keelu/R/FiFu_analysis/data/dynamics_df.csv")

result <- dynamics_df %>%
  group_by(stage, dynamic) %>%
  summarise(
    Means = mean(rate_ad),
    SDs = sd(rate_ad), count = n()
  )

# create gorup variable
result$group <- paste(result$stage, result$dynamic, sep="")

###### Fusion vs Growth

# filter the groups that I want to compare
selected_groups <- result %>% 
  filter(group %in% c("JFuM", "JGM", "JFuL", "JGL", "MFuL", "MGL"))
# set the levels of factors
selected_groups$group <- factor(selected_groups$group, 
                                levels = c("JFuM", "JGM", "JFuL", "JGL", "MFuL", "MGL"))
# set the comparing groups
selected_groups$gap_group <- factor(
  ifelse(selected_groups$group %in% c("JFuM", "JGM"), "J to M",
         ifelse(selected_groups$group %in% c("JFuL", "JGL"), "J to L", "M to L")),
  levels = c("J to M","J to L","M to L")
)

# compare Fu vs growth effect
fig7_1 <- ggplot(selected_groups, aes(x=group, y=Means, fill=group)) +
  geom_bar(stat="identity", position=position_dodge(width=0.8)) +
  facet_grid(~ gap_group, scales = "free_x", space = "free_x") +
  geom_errorbar(aes(ymin = Means - SDs, ymax = Means + SDs), position = position_dodge(width = 0.9), width = 0.25) +
  labs(title="Fusion vs Growth", x="Dynamics", y="Multiplier") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave("C:/Users/keelu/R/FiFu_analysis/Figs/Fusion_vs_Growth.png", plot = fig7_1, width = 5.3, height = 6, dpi = 100)


####### Fission vs shrinkage

# filter the groups that I want to compare
selected_groups <- result %>% 
  filter(group %in% c("MFiJ", "MSJ", "LFiJ", "LSJ", "LFiM", "LSM"))
# set the levels of factors
selected_groups$group <- factor(selected_groups$group, 
                                levels = c("MFiJ", "MSJ", "LFiJ", "LSJ", "LFiM", "LSM"))
# set the comparing groups
selected_groups$gap_group <- factor(
  ifelse(selected_groups$group %in% c("MFiJ", "MSJ"), "M to J",
         ifelse(selected_groups$group %in% c("LFiJ", "LSJ"), "L to J", "L to M")),
  levels = c("M to J","L to J","L to M")
)

# compare Fu vs growth effect
fig7_2 <- ggplot(selected_groups, aes(x=group, y=Means, fill=group)) +
  geom_bar(stat="identity", position=position_dodge(width=0.8)) +
  facet_grid(~ gap_group, scales = "free_x", space = "free_x") +
  geom_errorbar(aes(ymin = Means - SDs, ymax = Means + SDs), position = position_dodge(width = 0.9), width = 0.25) +
  labs(title="Fission vs Shrinkage", x="Dynamics", y="Multiplier") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
fig7_2
ggsave("C:/Users/keelu/R/FiFu_analysis/Figs/Fission_vs_Shrinkage.png", plot = fig7_2, width = 5.3, height = 6, dpi = 100)
```
...
change_ratio_J <- dynamics_df_J %>%
  group_by(dynamic) %>%
  summarize(mean_log = exp(mean(log(rate_ad))), mean = mean(rate_ad), count = n())
...
# Calculate multiplier for classification of P/A ratio and circularity
```{r}
########### Calculate multiplier

# Define the custom order
custom_order_2 <- c("GJ","GIA","GCA","SJ","SIA","SCA","FiJ","FiIA","FiCA","FuJ","FuIA","FuCA","D")

data <- dynamics_df_circ # chose the data set as P/A ratio or circularity

### Calculate mean change rate (multiplier) in each stage
dynamics_df_J <- filter(data, stage == "J" )
dynamics_df_IA <- filter(data, stage == "IA") 
dynamics_df_CA <- filter(data, stage == "CA")


change_ratio_J <- dynamics_df_J %>%
  group_by(dynamic) %>%
  summarize(mean_x = exp(mean(log(rate_ad))), count = n())
# Convert the Name column to a factor with the custom order
change_ratio_J$dynamic <- factor(change_ratio_J$dynamic, levels = custom_order_2)
# Arrange the tibble based on the custom order
change_ratio_J <- change_ratio_J %>%
  arrange(dynamic)
...
change_ratio_IA <- dynamics_df_IA %>%
  ...
  summarize(mean_x = exp(mean(log(rate_ad))), count = n())
# Convert the Name column to a factor with the custom order
change_ratio_IA$dynamic <- factor(change_ratio_IA$dynamic, levels = custom_order_2)
# Arrange the tibble based on the custom order
change_ratio_IA <- change_ratio_IA %>%
  arrange(dynamic)
...
change_ratio_CA <- dynamics_df_CA %>%
  ...
  summarize(mean_x = exp(mean(log(rate_ad))), count = n())
# Convert the Name column to a factor with the custom order
change_ratio_CA$dynamic <- factor(change_ratio_CA$dynamic, levels = custom_order_2)
# Arrange the tibble based on the custom order
change_ratio_CA <- change_ratio_CA %>%
  arrange(dynamic)
