...
##check total area
sum(pa_data$Shape_Area[pa_data$size_class_circ=="IA"])
sum(pa_data$Shape_Area[pa_data$size_class_circ=="CA"])
      ...
      for (i in 1:length(unique(pa_data_done_2$ID))) {
          ...
          if (pa_data_done_1$Fate[j] == "S" | pa_data_done_1$Fate[j] == "G"){
            if (current_id == pa_data_done_2$ID[i]){ # check the ID is match
              morpho_dynamic$stage[j] <- as.character(pa_data_done_1$size_class_circ[j])
              morpho_dynamic$dynamic[j] <- paste0(pa_data_done_1$Fate[j],pa_data_done_2$size_class_circ[i])
...
IA_t1 <- filter(pa_data_done, size_class_circ == "IA" & Survey_times == 1)
CA_t1 <- filter(pa_data_done, size_class_circ == "CA" & Survey_times == 1)
recruit_t2 <- filter(pa_data_done, Recruit == "Y" & Survey_times == 3)
IA_t2 <- filter(pa_data_done, size_class_circ == "IA" & Survey_times == 2)
CA_t2 <- filter(pa_data_done, size_class_circ == "CA" & Survey_times == 2)
...
df$size_class_circ <- factor(df$size_class_circ, levels = c("CA","IA","J"))
