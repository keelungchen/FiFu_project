...
for (i in 1:length(sort(unique(pa_data$pa_ratio)))){
  if(adult_area < half_adult_sum){
    ...
    cutoff_pa_ratio <- sort(unique(pa_data$pa_ratio))[i-1] #make CA slightly smaller
    break
  }
}
...
quantile(pa_data$pa_ratio[pa_data$size_class != "J"])


##Define PA ratio cutoff by Median P/A ratio
...
##check total area
sum(pa_data$Shape_Area[pa_data$size_class_pa=="IA"])
sum(pa_data$Shape_Area[pa_data$size_class_pa=="CA"])
...
p1_pa = ggplot(data = pa_data, aes(x = Survey_times, fill = size_class_pa,na.rm = TRUE)) + 
  geom_bar(position = "fill") +
  xlab("Survey_times") + 
  ylab("Proportion of colonies") 
p1_pa
      ...
      for (i in 1:length(unique(pa_data_done_2$ID))) {
          ...
          if (pa_data_done_1$Fate[j] == "S" | pa_data_done_1$Fate[j] == "G"){
            if (current_id == pa_data_done_2$ID[i]){ # check the ID is match
              morpho_dynamic$stage[j] <- as.character(pa_data_done_1$size_class_pa[j])
              morpho_dynamic$dynamic[j] <- paste0(pa_data_done_1$Fate[j],pa_data_done_2$size_class_pa[i])
...
dynamics_df_IA <- filter(dynamics_df, stage == "IA") #& Survey_times == 2 )
dynamics_df_CA <- filter(dynamics_df, stage == "CA")  #& Survey_times == 2)
...
IA_t1 <- filter(pa_data_done, size_class_pa == "IA" & Survey_times == 1)
CA_t1 <- filter(pa_data_done, size_class_pa == "CA" & Survey_times == 1)
recruit_t2 <- filter(pa_data_done, Recruit == "Y" & Survey_times == 3)
IA_t2 <- filter(pa_data_done, size_class_pa == "IA" & Survey_times == 2)
CA_t2 <- filter(pa_data_done, size_class_pa == "CA" & Survey_times == 2)
