...
adult_area <- 0
cutoff_size <- NA
adult_data <- pa_data[pa_data$size_class != "J", ]
...
plot(log10(pa_data$Shape_Area), (pa_data$Shape_Area),
     pch = 10,
     col = factor(pa_data$pure_size))
abline(v=log10(pi*0.03^2))#check the standard of adult
abline(h=0.7887669) #check the standard of compact adult

# Calculate number of J, IA, CA 
count_J_c <- sum(grepl("J", pa_data$pure_size))
count_IA_c <- sum(grepl("M", pa_data$pure_size))
count_CA_c <- sum(grepl("L", pa_data$pure_size))

count_IA_c
count_CA_c

##check total area
sum(pa_data$Shape_Area[pa_data$pure_size=="M"])
sum(pa_data$Shape_Area[pa_data$pure_size=="L"])
...
p1_c = ggplot(data = pa_data, aes(x = Survey_times, fill = pure_size,na.rm = TRUE)) + 
  geom_bar(position = "fill") +
  xlab("Survey_times") + 
  ylab("Proportion of colonies") 
p1_c
...
p2_c = ggplot(filter(pa_data_done, Survey_times == 1 | Survey_times == 2), aes(x = pure_size, fill = Fate,na.rm = TRUE)) + 
  geom_bar(position = "fill") +
  xlab("pure_size") + 
  ylab("Proportion of Fates")+
  geom_text(
      ...
      for (i in 1:length(unique(pa_data_done_2$ID))) {
          ...
          if (pa_data_done_1$Fate[j] == "S" | pa_data_done_1$Fate[j] == "G"){
            if (current_id == pa_data_done_2$ID[i]){ # check the ID is match
              morpho_dynamic$dynamic[j] <- paste0(pa_data_done_1$Fate[j],pa_data_done_2$pure_size[i])
...
dynamics_df$stage <- dynamics_df$pure_size
...
p2 = ggplot(data = subset(dynamics_df_1, !is.na(dynamic)), aes(x = factor(pure_size,levels = c("J","M","L")), fill = dynamic, na.rm = TRUE)) + 
  geom_bar(position = "fill") +
  geom_text(
    aes(label=signif(..count.. / tapply(..count.., ..x.., sum)[as.character(..x..)], digits=3)),
    stat="count",
...
dynamics_df_J <- filter(dynamics_df, stage == "J" )#& Survey_times == 2)
dynamics_df_IA <- filter(dynamics_df, stage == "M") #& Survey_times == 2 )
dynamics_df_CA <- filter(dynamics_df, stage == "L")  #& Survey_times == 2)
...
dynamics_df_J <- filter(dynamics_df, stage == "J" )#& Survey_times == 2) #& Survey_times == t
dynamics_df_IA <- filter(dynamics_df, stage == "M")# & Survey_times == 2)
dynamics_df_CA <- filter(dynamics_df, stage == "L")# & Survey_times == 2)
...
```{r}
###### Size
df_J <- filter(pa_data_done, pure_size=="J" )#& Survey_times == 3)
...
df$pure_size <- factor(df$pure_size, levels = c("L","M","J"))
